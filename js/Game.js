  ...
  END() {
    if(finishedCars<4)
    {
      textSize(20);
       stroke("black")
       fill("black")
    text("Game has ended now get lost", camera.position.x, camera.position.y )
    }
   }
   ...
   displayRank() 
   {
     ...
     text("LeaderBored",camera.position.x, camera.position.y-50);
    var display_position = camera.position.y;
    for(var plr in allPlayers){
